  ...
  return (
    <>
      <div className="topbar">
        ...
        <input placeholder="Filtrer…" value={q} onChange={(e) => setQ(e.target.value)} style={{ width: 240 }} />
      </div>
      {rows.length === 0 ? <Empty msg="Aucun événement." /> : (
        <div className="tablewrap"><table className="tbl">
          <thead><tr><th>Date</th><th>Acteur</th><th>Action</th><th>Entité</th><th>Détail</th></tr></thead>
          <tbody>{rows.map((a) => (
            <tr key={a.id}>
              <td style={{ whiteSpace: 'nowrap' }}>{new Date(a.created_at).toLocaleString('fr-FR')}</td>
              <td>{a.actor_staff_code ?? a.actor_role ?? 'système'}</td>
